...
contract ProtocolMainnet is Test {
    ...
    function setUp() public {
        ...
        ccipLocalSimulatorFork.setNetworkDetails(8453, base);
        ...
        ccipLocalSimulatorFork.setNetworkDetails(42161, arbitrum);
    ...
    function test_swapUniV3SingleMock() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Orchestrator_UnableToCompleteDelegateCall.selector,
                routerNotAllowed
            )
        );
        op.swap(swapData, User);
        ...
        bytes memory emptyDexData = abi.encodeWithSelector(DexSwap_EmptyDexData.selector);
        vm.expectRevert(
            abi.encodeWithSelector(Orchestrator_UnableToCompleteDelegateCall.selector, emptyDexData)
        );
        op.swap(swapData, User);
    }
    ...
    function test_swapUniV3MultiMock() public {
        ...
        bytes memory invalidPath = abi.encodeWithSelector(DexSwap_InvalidPath.selector);
        vm.expectRevert(
            abi.encodeWithSelector(Orchestrator_UnableToCompleteDelegateCall.selector, invalidPath)
        );
        op.swap(swapData, User);
    ...
    function test_revertSwapUniV3MultiMockInvalidPath() public {
        ...
        bytes memory encodedError = abi.encodeWithSelector(DexSwap_InvalidPath.selector);
        vm.expectRevert(
            abi.encodeWithSelector(Orchestrator_UnableToCompleteDelegateCall.selector, encodedError)
        );
        op.swap(swapData, User);
    ...
    function test_customMultiHopFunctionalityRevert() public {
        ...
        //==== Initiate transaction
        bytes memory notChained = abi.encodeWithSelector(
            DexSwap_SwapDataNotChained.selector,
            address(mUSDC),
            address(wEth)
        );

        vm.expectRevert(
            abi.encodeWithSelector(Orchestrator_UnableToCompleteDelegateCall.selector, notChained)
        );
        op.swap(swapData, User);
    }
    ...
    function test_swapAndBridgeMainnet() public setters {
        ...
        vm.expectRevert(abi.encodeWithSelector(Orchestrator_InvalidDstSwapData.selector));

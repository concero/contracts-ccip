...
const { buildRequestCBOR, decodeResult, SecretsManager, simulateScript, SubscriptionManager, Location, } = require("@chainlink/functions-toolkit");
const { types } = require("hardhat/config");
const CLFnetworks_1 = require("../../../constants/CLFnetworks");
const path = require("path");
const process = require("process");
task("clf-set-auto-request", "sets the CBOR-encoded Functions request in a deployed AutomatedFunctionsConsumer contract")
    ...
    .addOptionalParam("slotid", "Storage slot number 0 or higher. If the slotid is already in use, the existing secrets for that slotid will be overwritten.")
    .addOptionalParam("interval", "Update interval in seconds for Automation to call performUpkeep", 300, types.int)
    .addOptionalParam("ttl", "time to live - minutes until the secrets hosted on the DON expire. Defaults to 120m, and must be minimum 5m", 120, types.int)
    .addOptionalParam("gaslimit", "Maximum amount of gas that can be used to call fulfillRequest in the consumer contract", 250000, types.int)
...
const setAutoRequest = async (contract, taskArgs) => {
    ...
    if (taskArgs.simulate) {
        ...
        if (responseBytesHexstring) {
            console.log(`\nResponse returned by script during local simulation: ${decodeResult(responseBytesHexstring, requestConfig.expectedReturnType)}\n`);
        }
        if (errorString) {
    ...
    if (network.name !== "localFunctionsTestnet" &&
        ...
        if (requestConfig.secretsLocation !== Location.DONHosted) {
            throw Error(`\nThis task supports only DON-hosted secrets. The request config specifies ${Location[requestConfig.secretsLocation]}.`);
        }
        ...
        const { version, success } = await secretsManager.uploadEncryptedSecretsToDON({
            ...
            gatewayUrls: CLFnetworks_1.networks[network.name]["gatewayUrls"],
            slotId,
    ...
    const setRequestTx = await autoConsumerContract.setRequest(taskArgs.subid, taskArgs.gaslimit, taskArgs.interval, functionsRequestCBOR);
    console.log(`\nWaiting ${CLFnetworks_1.networks[network.name].confirmations} block for transaction ${setRequestTx.hash} to be confirmed...`);
    await setRequestTx.wait(CLFnetworks_1.networks[network.name].confirmations);

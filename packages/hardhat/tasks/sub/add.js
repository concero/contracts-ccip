...
(0, config_1.task)("clf-sub-consumer-add", "Adds a consumer contract to the Functions billing subscription")
    ...
    let subscriptionId;
    if (!taskArgs.subid) {
        console.log(`No subscription ID provided, defaulting to ${CNetworks_1.default[name].functionsSubIds[0]}`);
        subscriptionId = CNetworks_1.default[name].functionsSubIds[0];
    }
    else
        subscriptionId = parseInt(taskArgs.subId);
    const consumerAddresses = taskArgs.contract.split(",");
    await addCLFConsumer(CNetworks_1.default[name], consumerAddresses, subscriptionId);
});

...
contract ConceroFunctions is FunctionsClient, IFunctions, ConceroCommon {
  ...
  string private constant srcJsCode =
    "try { await import('npm:ethers@6.10.0'); return eval(secrets.SRC_JS); } catch (err) { throw new Error(err.message.slice(0, 255));}";
  string private constant dstJsCode =
    "try { await import('npm:ethers@6.10.0'); return eval(secrets.DST_JS); } catch (err) { throw new Error(err.message.slice(0, 255));}";

...
return (async () => {
  try {
    ...
    console.log('srcContractAddress:', srcContractAddress);
    ...
    const ethersId = ethers.id('CCIPSent(bytes32,address,address,uint8,uint256,uint64,bytes32)');
    ...
    const log = logs[0];
    const abi = ['event CCIPSent(bytes32 indexed, address, address, uint8, uint256, uint64, bytes32)'];

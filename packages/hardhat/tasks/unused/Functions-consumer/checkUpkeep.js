...
task("clf-check-upkeep", "Checks if checkUpkeep returns true for an Automation compatible contract")
    .addParam("contract", "Address of the contract to check")
    .addOptionalParam("data", "Hex string representing bytes that are passed to the checkUpkeep function (defaults to empty bytes)")
    .setAction(async (taskArgs) => {
    const hre = require("hardhat");
    const checkData = taskArgs.data ?? [];
    console.log(`Checking if upkeep is required for Automation consumer contract ${taskArgs.contract} on network ${network.name}`);

...
(0, config_2.task)("clf-set-donid", "Updates the oracle address for a FunctionsConsumer consumer contract using the FunctionsOracle address from `network-config.js`")
    .addParam("contract", "Address of the consumer contract to update")
    .setAction(async (taskArgs) => {
    const { name } = hre.network;
    const donId = CNetworks_2.default[name].functionsDonId;
    ...
    console.log(`\nWaiting ${CNetworks_2.default[name].confirmations} blocks for transaction ${updateTx.hash} to be confirmed...`);
    await updateTx.wait(CNetworks_2.default[name].confirmations);

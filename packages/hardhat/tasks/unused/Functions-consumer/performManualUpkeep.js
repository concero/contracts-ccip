...
task("clf-perform-upkeep", "Manually call performUpkeep in an Automation compatible contract")
    .addParam("contract", "Address of the contract to call")
    .addOptionalParam("data", "Hex string representing bytes that are passed to the performUpkeep function (defaults to empty bytes)")
    .setAction(async (taskArgs) => {
    // A manual gas limit is required as the gas limit estimated by Ethers is not always accurate
    const overrides = {
        gasLimit: 1000000,
        gasPrice: CLFnetworks_1.networks[network.name].gasPrice,
    };
    // Call performUpkeep
    ...
    console.log(`\nCalling performUpkeep for Automation consumer contract ${taskArgs.contract} on network ${network.name}${taskArgs.data ? ` with data ${performData}` : ""}`);
    const performUpkeepTx = await autoConsumerContract.performUpkeep(performData, overrides);
    console.log(`\nWaiting ${CLFnetworks_1.networks[network.name].confirmations} blocks for transaction ${performUpkeepTx.hash} to be confirmed...`);
    const events = (await performUpkeepTx.wait(CLFnetworks_1.networks[network.name].confirmations)).events;

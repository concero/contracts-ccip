...
task("functions-set-auto-request", "sets the CBOR-encoded Functions request in a deployed AutomatedFunctionsConsumer contract")
...
const setAutoRequest = async (contract, taskArgs) => {
  ...
  if (network.name !== "localFunctionsTestnet" && requestConfig.secrets && Object.keys(requestConfig.secrets).length > 0) {

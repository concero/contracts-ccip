...
task("functions-request", "Initiates an on-demand request from a Functions consumer contract")
  ...
  .addOptionalParam("simulate", "Flag indicating if source JS should be run locally before making an on-chain request", true, types.boolean)
  .addOptionalParam("callbackgaslimit", "Maximum amount of gas that can be used to call fulfillRequest in the consumer contract", 100_000, types.int)
  .addOptionalParam(
    "slotid",
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.simulate) {
      ...
      if (responseBytesHexstring) {
        console.log(
          `\nResponse returned by script during local simulation: ${decodeResult(responseBytesHexstring, requestConfig.expectedReturnType).toString()}\n`,
        );
      }
      if (errorString) {
    ...
    spinner.start(`Waiting for transaction for FunctionsConsumer contract ${contractAddr} on network ${network.name} to be confirmed...`);
    ...
    try {
      // Get response data
      const { requestId, totalCostInJuels, responseBytesHexstring, errorString, fulfillmentCode } = await responseListener.listenForResponseFromTransaction(
        requestTx.hash,
      );
      ...
      switch (fulfillmentCode) {
        ...
        default:
          spinner.fail(`Request ${requestId} failed with fulfillment code: ${fulfillmentCode}. Please contact Chainlink support.`);
      }
    } catch (error) {
